...
bool createImageBmp(FILE *fh, size_t width, size_t height, bool splitUp)
{
	...
	if (!writeBitmapHeader24(fh, width, height)) {
		return false;
	}
	if (splitUp) {
		...
		for (int i = 0; i < gBmpHeight; ++i) {
			if ((int)fwrite(tmpdata, 1, gBmpLineWidth, fh) != gBmpLineWidth) {
				return false;
			}
		}
		delete[] tmpdata;
...
bool loadImagePartBmp(FILE *fh, int startx, int starty, int width, int height)
{
	...
	if (width + startx > gBmpWidth) {
		width = gBmpWidth - startx;
	}
	if (height + starty > gBmpHeight) {
		height = gBmpHeight - starty;
	}
	...
	if (gBitmap == NULL) {
		...
		for (int arrayLine = 0; arrayLine < gBmpLocalHeight - offY; ++arrayLine) {
			...
			if ((int)fread(gBitmap + (arrayLine * gBmpLocalLineWidth) + offX * 3, 1, readLineWidth, fh) != readLineWidth) {
				return false;
			}
			++fileLine;
		}
...
bool saveImagePartBmp(FILE *fh)
{
	...
	for (int arrayLine = 0; arrayLine < gBmpLocalHeight - offY; ++arrayLine) {
		...
		fseek64(fh, pos, SEEK_SET);
		if ((int)fwrite(gBitmap + (arrayLine * gBmpLocalLineWidth) + offX * 3, 1, writeLineWidth, fh) != writeLineWidth) {
			return false;
		}
		++fileLine;
	}
