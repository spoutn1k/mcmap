...
static int Utf8ToWideChar(char *pUTF8, size_t cchSrc, wchar_t *pWCHAR,
                          size_t cchDest, bool *isvalid) {
  if (!pUTF8 || !pWCHAR) {
...
DIRHANDLE open(char *path, myFile &file) {
  ...
  WideCharToUtf8(ffd.cFileName, wcslen(ffd.cFileName), (uint8_t *)file.name,
                 sizeof(file.name));
  file.isdir = ((ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) ==
