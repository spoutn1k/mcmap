...
union Translator {
  ...
  Translator() : buffer(0){};
  Translator(const uint8_t *_bytes, DataType type) : buffer(0) {
    switch (type) {
    case SHORT:
      ((uint8_t *)&buffer)[1] = _bytes[0];
      ((uint8_t *)&buffer)[0] = _bytes[1];
      break;

    case INT:
    case FLOAT:
      for (uint8_t i = 0; i < 4; i++)
        ((uint8_t *)&buffer)[i] = _bytes[4 - i - 1];
      break;

    case LONG:
    case DOUBLE:
      for (uint8_t i = 0; i < 8; i++)
        ((uint8_t *)&buffer)[i] = _bytes[8 - i - 1];
      break;
    }
  }
  ...
  uint8_t *bytes() { return (uint8_t *)&buffer; }
};
